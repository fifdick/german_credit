...
p1 <- evalm(list(xgb_train_1$fit, xgb_train_up, xgb_train_smote, xgb_train_down),
...
# predict on test
pred <- predict(xgb_train_1, newdata = test_x %>% as.matrix)
pred
#test_y$Diagnosis %>% as.factor -> test_y
tab <- caret::confusionMatrix(pred, test_y)
tab
caret::precision(tab$table)
caret::recall(tab$table)
caret::F_meas(tab$table, relevant = "bad", beta = 1.5)
