...
# The same applies to status (of checking account)
	...
	select(-c(job, housing, other_installment_plans, property, other_debtors, personal_status_sex, purpose, credit_history)) %>%
...
fb <- function(data, lev = NULL, model = NULL) {
  f1_val <- FBeta_Score(y_pred = data$pred, y_true = data$obs, positive = lev[2], beta = 1.5)
...
#
saveRDS(xgb_train_1, "xgb1.rds")
saveRDS(xgb_train_up, "xgbup.rds")
saveRDS(xgb_train_down, "xgbdown.rds")
saveRDS(xgb_train_smote, "xgbsmote.rds")


#plot tuning based on f1
plot(xgb_train_1)
xgb_train_1$results %>% arrange(desc(Fbeta)) %>% head(1)
plot(xgb_train_down)
xgb_train_down$results %>% arrange(desc(Fbeta)) %>% head(1)
plot(xgb_train_up) 
xgb_train_up$results %>% arrange(desc(Fbeta)) %>% head(1)
plot(xgb_train_smote) 
xgb_train_smote$results %>% arrange(desc(Fbeta)) %>% head(1)

#plot roc when classProb = true plot all three models in one
p1 <- evalm(list(xgb_train_1, xgb_train_up, xgb_train_smote, xgb_train_down),
      gnames = c('imb','upsampled', 'smote', 'downsampled'), optimise = "F1", 
      rlinethick = 0.8, fsize = 8, plots = 'proc',
	positive = "bad")
p1$proc
p1$roc
...
base_f <-  sapply(xgb_train_smote$control$indexOut, function(cv){
	true = train_y[cv]
	true
	levels(true)
	...
	caret::F_meas(tab$table, relevant = "bad", beta = 1.5)
})
mean(base_f)

#variImp
varImp(xgb_train_1)$importance %>% as.data.frame %>%
	mutate(Sampling = "none") %>%
bind_rows(varImp(xgb_train_down)$importance %>% as.data.frame %>% mutate(Sampling = "down")) %>%
bind_rows(varImp(xgb_train_up)$importance %>% as.data.frame %>% mutate(Sampling = "up")) %>%
bind_rows(varImp(xgb_train_smote)$importance %>% as.data.frame %>% mutate(Sampling = "smote")) %>% 
mutate(Feature = gsub("...\\d+", "", rownames(.))) %>%
mutate(Sampling = factor(Sampling))    -> df_vars
# summarise to calculate overall mean across models and use levels of feature to sort
df_vars %>%  
	group_by(Feature) %>%
	summarise(mean_imp = mean(Overall)) %>%
	arrange(mean_imp) -> order_f
df_vars <- df_vars %>%
	mutate(Feature = factor(Feature, levels = order_f$Feature))

ggplot(df_vars, aes(x = Overall, y = Feature, fill = Sampling)) +
geom_bar(position = "dodge", stat = "identity") +
...
# predict on test
pred <- predict(xgb_train_smote, newdata = test_x %>% as.matrix, )
